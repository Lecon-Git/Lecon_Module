...
def divide(arg1, arg2):
    ...
    if arg2 == 0:  # Check for zeroerror
        raise ZeroDivisionError("Zero division error, check your denominator.")
    return arg1 / arg2
...
def sine(arg1, arg2):
    ...
    ###
    return divide(float(arg1),float(arg2))
...
def areaOfCircle(arg):
    return multiply(PIE, square(arg))
...
def nth_root(arg1, arg2):
    ...
    # This function compute nth power of a given integer, n and where p is the power to which it raises
    # return square of the number
    return arg1 ** divide(float(1),float(arg2))
...
def sigmoid(arg):
    ...
    ###
    return divide(float(1),float(1+exp(arg)))
...
def binarySearch(arrayArg, key):
    ...
    if length == 1:
        return True if key == arrayArg[0] else False
...
# Implementation number system conversions
# 
# Decimal to Binary: Dec2Bin

dividends = []
def convertToBaseX(n, base):
    # takes in decimal argument
    # return a binary representation
    # 
    # Algorithm
    # - divide input n, by 2 and keep track of the result by appending to string variable
    # - then reverse the appended string variable content
    global dividends
    divisor = base
    if n == 0 or n == 1:
        dividends.append(str(n))
        print("================================    ================================\n")
        print(" To use this function again, global variable that stores the result needs to be cleared. \
              \n\n Call 'clear() func on the library name e.g lecon.clear()")
        print("\n====================================================================")
        return ''.join([i for i in reversed(dividends)])
    dividend = n % divisor
    dividends.append(str(dividend))
    n //= divisor
    return convertToBaseX(n, divisor)

def baseXToDecimal(n, base):
    pow = len(n) - 1
    values = list(n)
    return sum([int(value) * (base ** (pow - key)) for key, value in enumerate(values)])
