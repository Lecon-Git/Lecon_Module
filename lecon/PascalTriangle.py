...
class Pascal:
    ...
    ###
    def __init__(self, firstTerm=None, secondTerm=None, index=0):
        ###
        # (a + (+b|-b))^n
        # Argument:
        #   firstTerm (int) => a
    ...
    def print_tree(self):
        size = len(self.pascal_tree)
        for i in range(0, size):
            for _ in range(1, size-i):
                print(" ", end="")
            print(self.pascal_tree[i])
    ...
    def computeCoefficient(self):
        ...
        pascal_tree_len = len(self.pascal_tree)
        i = 2
            ...
            for j in range(0, spog.add(i, 1)):
                if i < pascal_tree_len and j <= index:
                    if (j == 0 or j == i):
                        ...
                        self.pascal_tree[i].append(1)
                    else:
                        # append sum of the previous of the list to next of the element,
                        # to get immediate element of the list
                        self.pascal_tree[i].append(
                            spog.add(self.pascal_tree[i-1][j-1], self.pascal_tree[i-1][j]))
        ...
        self.coefficients = self.pascal_tree[index-1]  # store the last list.
    ...
    def computeFirstTerm(self):
        ...
        self.firstTermCollection = firstT
    ...
    def computeSecondTerm(self):
        ...
        self.secondTermCollection = secondT
    ...
    def pascalSolution(self):
        ...
        if self.fTerm is None:
            raise ValueError("First term cannot be None")
        if self.sTerm is None:
            raise ValueError("Second term cannot be None")
